...
- **Soft ceiling of 250 characters per prose line.** When a single-paragraph line exceeds 250 characters, split it at the last safe sentence boundary that keeps each chunk at or below 250 characters. A safe boundary is `[.!?]` followed by a space and an uppercase letter. Headings, list items, table rows, blockquotes, code fences, indented code blocks, and YAML frontmatter are exempt and are passed through verbatim. Lines that contain no safe boundary (long URL, hash reference, ...) are left untouched rather than broken mid-word. Apply with `python3 _helpers/scripts/format-prose.py <file-or-dir> [...]`; dry-run with `--check` (exit 1 on diff); override the ceiling via `MAX=200 python3 _helpers/scripts/format-prose.py ...`. A directory target is walked recursively for `*.md` files; hidden directories and `node_modules` / `dist` / `out` / `__pycache__` / `.venv` / `venv` / `_Private` are pruned automatically. Formatter lives in `_helpers/scripts/format-prose.py` alongside the other public utility scripts.
- **ATX headings** (`#`, `##`).
