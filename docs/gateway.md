...
| `aiflowbridge.gateway.injectThoughtSignature`        | `false`                       | When `true`, re-injects cached `thought_signature` values into the Gemini / Antigravity envelope when the client replays a tool turn without `extra_signature`. Default `false` keeps the pure pass-through contract (client-supplied signatures propagate, missing ones stay missing). Enable when the OpenAI-compatible client drops unknown `extra_signature` fields from its persisted history (e.g. Kilo Code CLI `openai-chat` protocol) - the gateway then closes the gap server-side from its in-memory cache (bounded to 500 entries, 30 min TTL) without the client changing anything |
