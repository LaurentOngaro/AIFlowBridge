...
export function processToolFlow({ stabilizeToolList, messages, tools, progress, requestKind }: ToolFlowOptions): ToolFlowResult {
  ...
  if (activatePreflight.remainingActivatorNames.length > 0) {
    if (activatePreflight.rounds >= MAX_PREFLIGHT_ROUNDS_PER_USER_REQUEST) {
      ...
      throw new Error(t('request.preflightRoundLimitExceeded', MAX_PREFLIGHT_ROUNDS_PER_USER_REQUEST));
    }
  ...
  const hasUnexpandedActivateTools = activatePreflight.rounds > 0 && tools?.some((tool) => tool.name.startsWith(ACTIVATE_TOOL_PREFIX));
  logToolFlowDiagnostics({
    requestKind,
    tools,
    messagesFiltered,
