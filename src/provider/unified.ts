...
export class UnifiedChatProvider implements vscode.LanguageModelChatProvider {
  ...
  async provideLanguageModelChatResponse(
    ...
    try {
      await entry.provider.provideLanguageModelChatResponse(modelInfo, messages, options, progress, token);
    } catch (error) {
  ...
  private async resolveHasApiKey(provider: AnyProvider): Promise<boolean> {
    ...
    if (typeof (provider as { getAuthManager?: () => { hasApiKey(): Promise<boolean> } }).getAuthManager === 'function') {
      return (provider as { getAuthManager: () => { hasApiKey(): Promise<boolean> } }).getAuthManager().hasApiKey();
    }
    return false;
  }
}
