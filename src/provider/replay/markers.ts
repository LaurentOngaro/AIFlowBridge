...
export function findFirstReplayMarker(message: vscode.LanguageModelChatRequestMessage): LocatedReplayMarker | undefined {
...
export function parseFirstReplayMarker(message: vscode.LanguageModelChatRequestMessage): ReplayMarkerParseResult | undefined {
...
export function createReplayMarkerPart(metadata: ReplayMarkerMetadata): vscode.LanguageModelDataPart {
  ...
  return new vscode.LanguageModelDataPart(new TextEncoder().encode(`${REPLAY_MARKER_WRITER_ID}\\${payload}`), REPLAY_MARKER_MIME);
}
...
): { valid: true; value: string; format: ReplayMarkerPayloadFormat } | { valid: false; error: string } {
