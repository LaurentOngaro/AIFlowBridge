...
import { classifyDeepSeekRequest, formatModelFields, formatRequestLogLine, type RequestKind } from './classifier';
...
export function logToolFlowDiagnostics({
  ...
  const activateToolCount = tools?.reduce((count, tool) => count + (tool.name.startsWith(ACTIVATE_TOOL_PREFIX) ? 1 : 0), 0) ?? 0;
  if (preflight === 'skipped' && !messagesFiltered && activateToolCount === 0) {
    return;
  }

  let message = `[tool-flow] preflight=${preflight}` + ` tools=${tools?.length ?? 0}` + ` activateTools=${activateToolCount}`;
  if (messagesFiltered) {
...
class DefaultCacheDiagnosticsRecorder implements CacheDiagnosticsRecorder {
  ...
  beginRequest(options: BeginCacheDiagnosticsOptions): CacheDiagnosticsRun {
    ...
    const cacheTrace = createCacheTraceSnapshot(options.request, options.inputMessages, requestKind);
    ...
    const previousScopedCacheTrace = this.lastCacheTracesByScope.get(getCacheTraceScopeKey(cacheTrace));
    const cacheTraceComparison = compareCacheTraceSnapshots(previousCacheTrace, cacheTrace);
    const immediateTraceKeyChanged =
      previousImmediateCacheTrace?.cacheTraceKey !== undefined && previousImmediateCacheTrace.cacheTraceKey !== cacheTrace.cacheTraceKey;
    ...
    const skippedImmediateFallback =
      previousImmediateCacheTrace && immediateTraceKeyChanged && !sameImmediateComparisonScope ? previousImmediateCacheTrace : undefined;
    ...
    logger.info(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] ${formatCacheTraceSnapshot(cacheTrace)}`));
    logger.info(
    ...
    if (shouldLogHostPromptTrace(hostPromptTrace)) {
      logger.info(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] ${formatHostPromptTrace(hostPromptTrace)}`));
    }
    const vscodeMessageTrace = formatVscodeMessageTrace(options.inputMessages);
    if (vscodeMessageTrace) {
      logger.debug(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] vscodeMsgs ${vscodeMessageTrace}`));
    }
    for (const detailLine of formatCacheTraceDetailLines(cacheTrace)) {
      const message = formatRequestLogLine(requestKind, `[cache-trace #${requestId}] ${detailLine}`);
    ...
    if (cacheTraceComparison) {
      logger.info(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] ${formatCacheTraceComparison(cacheTraceComparison)}`));
    ...
    if (traceKeyChangeComparison && previousImmediateCacheTrace) {
      ...
      for (const detailLine of formatCacheTraceComparisonDetailLines(traceKeyChangeComparison)) {
        logger.info(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] cacheTraceKeyChanged ${detailLine}`));
      }
      for (const warning of getCacheTraceComparisonWarnings(traceKeyChangeComparison)) {
        logger.warn(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] cacheTraceKeyChanged fallback diff: ${warning}`));
      }
    }
    ...
    if (scopedTraceKeyChangeComparison && previousScopedCacheTrace) {
      ...
      for (const detailLine of formatCacheTraceComparisonDetailLines(scopedTraceKeyChangeComparison)) {
        logger.info(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] sameScope cacheTraceKeyChanged ${detailLine}`));
      }
      for (const warning of getCacheTraceComparisonWarnings(scopedTraceKeyChangeComparison)) {
        logger.warn(formatRequestLogLine(requestKind, `[cache-trace #${requestId}] sameScope cacheTraceKeyChanged fallback diff: ${warning}`));
      }
    }
...
class ActiveCacheDiagnosticsRun implements CacheDiagnosticsRun {
  ...
  onCancellationTokenRequested(): void {
    ...
    logger.info(formatRequestLogLine(this.snapshot.requestKind, `[cache-trace #${this.requestId}] cancellation token requested; aborting stream`));
  }

  onReplayMarkerReport(info: ReplayMarkerReportInfo): void {
    logger.info(formatRequestLogLine(this.snapshot.requestKind, `[cache-trace #${this.requestId}] ${formatReplayMarkerReport(info)}`));
  }
}
...
function formatReplayMarkerReport(info: ReplayMarkerReportInfo): string {
  ...
  return `replayMarker status=${info.status}` + trigger + markerBytes + visionTextChars + reasoningTextChars + reason + error;
}

function summarizeHostPromptTrace(messages: readonly vscode.LanguageModelChatRequestMessage[]): HostPromptTrace {
...
function getHostFreezeCustomizationsIndex(): boolean | 'unknown' {
  const value = vscode.workspace.getConfiguration('github.copilot.chat').get<unknown>('freezeCustomizationsIndex');
...
function logUsage(usage: DeepSeekUsage, charsPerToken: number, context: UsageLogContext, requestId?: number): void {
...
function summarizeVisionResolution(
  ...
  for (const [index, message] of inputMessages.entries()) {
    ...
    if (imageParts > 0) {
      ...
      const newDescriptions = Math.max(0, countLiteral(resolvedText, '[Image Description:') - countLiteral(inputText, '[Image Description:'));
      const newFailures = Math.max(
        0,
        countLiteral(resolvedText, IMAGE_DESCRIPTION_UNAVAILABLE) - countLiteral(inputText, IMAGE_DESCRIPTION_UNAVAILABLE)
      );
...
function formatVisionTrace(stats: VisionMessageStats, pipelineStats: VisionPipelineStats | undefined): string | undefined {
  if (stats.inputImageParts === 0 && stats.historyDescriptionMessages === 0 && !hasVisionPipelineActivity(pipelineStats)) {
    return undefined;
  }
  ...
  const parts = [`vision inputImages=${stats.inputImageParts}`, `inputMessages=${stats.inputImageMessages}`];
...
function formatVisionModel(stats: VisionMessageStats): string {
  ...
  if (stats.droppedImageParts > 0 && stats.describedImageMessages === 0 && stats.failedImageMessages === 0) {
    return 'none';
  }
  return 'unknown';
}

function formatVscodeMessageTrace(messages: readonly vscode.LanguageModelChatRequestMessage[]): string | undefined {
...
function summarizeInputAssistantMessages(messages: readonly vscode.LanguageModelChatRequestMessage[]): CacheTraceInputAssistantSummary[] {
  ...
  for (const [index, message] of messages.entries()) {
    ...
    for (const part of message.content) {
      ...
      } else if (part instanceof vscode.LanguageModelDataPart && part.mimeType === REPLAY_MARKER_MIME) {
        replayMarkerParts += 1;
      }
    }
...
export function compareCacheTraceSnapshots(previous: CacheTraceSnapshot | undefined, current: CacheTraceSnapshot): CacheTraceComparison | undefined {
  if (!previous) {
    return undefined;
  }
  ...
  const previousMessage = firstChangedMessageIndex === undefined ? undefined : previous.messageSummaries[firstChangedMessageIndex];
  const currentMessage = firstChangedMessageIndex === undefined ? undefined : current.messageSummaries[firstChangedMessageIndex];

  return {
    commonPrefixSummaryChars,
    commonPrefixSummaryPercent:
      current.redactedComparisonInput.length > 0 ? (commonPrefixSummaryChars / current.redactedComparisonInput.length) * 100 : 100,
    ...
    previousTool: firstChangedToolIndex === undefined ? undefined : previous.toolSummaries[firstChangedToolIndex],
    currentTool: firstChangedToolIndex === undefined ? undefined : current.toolSummaries[firstChangedToolIndex],
    systemPromptChange: firstChangedMessageIndex === 0 ? compareSystemPromptSections(previousMessage, currentMessage) : undefined,
  };
}
...
export function formatCacheTraceComparison(comparison: CacheTraceComparison): string {
  ...
  const changedTool = comparison.toolsChanged
    ? ` toolsHash=${comparison.previousToolsHash}->${comparison.currentToolsHash}` + ` firstChangedTool=${formatChangedTool(comparison)}`
    : '';
...
export function formatCacheTraceComparisonDetailLines(comparison: CacheTraceComparison): string[] {
  if (comparison.firstChangedMessageIndex === undefined || !comparison.previousMessage || !comparison.currentMessage) {
    return [];
  }
...
function countEmptyReasoningFallbackMessages(snapshot: CacheTraceSnapshot, cause: EmptyReasoningFallbackCause): number {
  let count = 0;
  ...
  for (const message of snapshot.messageSummaries) {
    ...
    if (isReasoningRequiredEmptyFallback(message) && classifyEmptyReasoningFallback(snapshot, assistantOrdinal) === cause) {
      count += 1;
    }
    assistantOrdinal += 1;
  }
...
function classifyEmptyReasoningFallback(snapshot: CacheTraceSnapshot, assistantOrdinal: number): EmptyReasoningFallbackCause {
  const inputAssistant = snapshot.inputAssistantSummaries[assistantOrdinal];
  if (!inputAssistant) {
    return 'unknown';
  }
  if (inputAssistant.thinkingChars > 0 || inputAssistant.replayMarkerReasoningChars > 0 || inputAssistant.replayMarkerInvalid) {
...
function compareSystemPromptSections(
  ...
  for (let index = 0; index < maxLength; index += 1) {
    ...
    if (previousSection?.hash !== currentSection?.hash || previousSection?.label !== currentSection?.label) {
      return {
...
function formatSystemPromptChange(change: CacheTraceSystemPromptChange): string {
  const changedSection =
    change.firstChangedSectionIndex === undefined ? 'firstChangedSection=unknown' : `firstChangedSection=${change.firstChangedSectionIndex}`;
  return (
    `${changedSection}` +
...
function summarizeMessage(message: DeepSeekMessage, index: number, followsToolResult: boolean): CacheTraceMessageSummary {
  ...
  const afterToolResultKind = assistantAfterToolResult ? (toolCalls > 0 ? ('tool-call' as const) : ('final' as const)) : ('none' as const);
...
function formatToolSummary(summary: CacheTraceToolSummary | undefined): string {
  ...
  return `${summary.name}#${summary.index}` + ` hash=${summary.hash}` + ` desc=${summary.descriptionHash}` + ` params=${summary.parametersHash}`;
}

function findFirstChangedMessageIndex(previous: CacheTraceMessageSummary[], current: CacheTraceMessageSummary[]): number | undefined {
...
function findFirstChangedToolIndex(previous: CacheTraceToolSummary[], current: CacheTraceToolSummary[]): number | undefined {
  const maxLength = Math.max(previous.length, current.length);
  for (let index = 0; index < maxLength; index += 1) {
    if (previous[index]?.hash !== current[index]?.hash) {
      return index;
