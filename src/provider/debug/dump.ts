...
import { classifyDeepSeekRequest, classifyProviderRequest, formatModelFields, formatRequestLogLine, type RequestKind } from './classifier';
 ...
 */
export function dumpDeepSeekRequest(request: DeepSeekRequest, options: DumpDeepSeekRequestOptions): void {
  if (!getRequestDumpEnabled()) return;
  ...
  const context = createDumpContext(options.globalStorageUri, options.segment, 'deepseek-request', (dumpCounter += 1), requestKind);
...
function createProviderInputSnapshot(options: DumpProviderInputOptions, context: DumpContext): object {
...
function serializeMessage(message: vscode.LanguageModelChatRequestMessage, index: number): SerializedMessage {
...
function serializeContentPart(part: unknown, index: number): SerializedContentPart {
  ...
  if (part instanceof vscode.LanguageModelDataPart) {
    const replayMarker = part.mimeType === REPLAY_MARKER_MIME ? summarizeReplayMarker(parseReplayMarkerData(part.data)) : undefined;
    return {
      index,
      type: 'data',
...
function serializeTools(tools: readonly vscode.LanguageModelChatTool[] | undefined): object[] | undefined {
...
function summarizeMessagesFromInput(messages: readonly vscode.LanguageModelChatRequestMessage[]): object {
  return summarizeMessages(messages.map((message, index) => serializeMessage(message, index)));
}

function summarizeVscodeSystemPrompt(messages: readonly vscode.LanguageModelChatRequestMessage[]): SystemPromptSummary {
...
function summarizeVscodeCustomizations(messages: readonly vscode.LanguageModelChatRequestMessage[]): CustomizationsSummary {
...
function summarizeDeepSeekCustomizations(messages: readonly DeepSeekMessage[]): CustomizationsSummary {
  let customizationsUpdateCountInHistory = 0;
  let latestUserMessageIndex: number | null = null;
  let latestUserHasCustomizationsUpdate = false;
...
async function writeJsonFile<T>(filePath: string, value: T, stringify: (value: T) => string = safeStringify): Promise<string> {
...
async function writeDumpObservation(globalStorageUri: vscode.Uri, observation: object): Promise<void> {
  const baseRoot = getRequestDumpBaseRoot(globalStorageUri);
  await mkdir(baseRoot, { recursive: true });
  await appendFile(join(baseRoot, REQUEST_OBSERVATIONS_FILE), `${safeStringify(observation)}\n`, 'utf-8');
}
