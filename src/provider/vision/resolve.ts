...
export async function resolveImageMessages(
  ...
  for (const [messageIndex, message] of messages.entries()) {
    ...
    if (replayText) {
      ...
      result.push(createResolvedMessage(message, [...nonImageParts, new vscode.LanguageModelTextPart(replayText)]));
      continue;
    }
...
function collectInputImageStats(messages: readonly vscode.LanguageModelChatRequestMessage[], stats: VisionResolutionStats): void {
...
function createVisionMarkerBindings(messages: readonly vscode.LanguageModelChatRequestMessage[], stats: VisionResolutionStats): Map<number, string> {
...
function findAssistantVisionText(message: vscode.LanguageModelChatRequestMessage, stats: VisionResolutionStats): string | undefined {
...
async function describeImageParts(
  ...
  const visionMsg = vscode.LanguageModelChatMessage.User([...parts, new vscode.LanguageModelTextPart(visionPrompt)] as (
...
function createVisionReplayText(visionText: string, nonImageParts: readonly vscode.LanguageModelInputPart[]): string {
...
function getNonImageParts(message: vscode.LanguageModelChatRequestMessage): vscode.LanguageModelInputPart[] {
  return (message.content as readonly vscode.LanguageModelInputPart[]).filter((part) => !isImageDataPart(part));
}

function hasNonEmptyTextPart(parts: readonly vscode.LanguageModelInputPart[]): boolean {
  return parts.some((part) => part instanceof vscode.LanguageModelTextPart && part.value.trim().length > 0);
}
