import { t } from '../i18n';
import { getApiProviderHttpErrorLinks, MAX_DIAGNOSTIC_FIELD_LENGTH, NETWORK_ERROR_CATEGORY_BY_CODE, OFFICIAL_DEEPSEEK_API_HOST } from './consts';
import type {
  ApiProviderId,
...
export async function createHttpError(response: Response, baseUrl: string): Promise<DeepSeekRequestError> {
  ...
  const diagnosticMessage = joinDiagnosticParts(
    ...
    responseText && responseText !== serverMessage ? `body=${truncateSingleLine(responseText)}` : undefined
  );
...
export function normalizeRequestError(error: unknown): Error {
  ...
  const enhanced = new DeepSeekRequestError({
    message: code ? `DeepSeek request failed due to network error ${code}` : 'DeepSeek request failed due to a network error',
    userSummary,
    kind: 'network',
    code,
    cause: error,
...
function getHttpErrorMessage(status: number, createApiKeyUrl?: string): string {
  switch (status) {
    ...
    case 401:
      return createApiKeyUrl ? t('error.http.401.withCreateApiKeyLink', status, createApiKeyUrl) : t('error.http.401', status);
    case 402:
...
function isKnownNetworkErrorCode(code: string): code is keyof typeof NETWORK_ERROR_CATEGORY_BY_CODE {
...
function extractServerMessage(responseText: string): string | undefined {
  ...
  try {
    ...
    const message = getStringProperty(error, 'message') ?? getStringProperty(parsed, 'message') ?? (typeof error === 'string' ? error : undefined);
...
function getCauseInfo(error: Error): { code?: string; name?: string; message?: string } | undefined {
  ...
  if (cause instanceof Error) {
    return {
      ...
      name: cause.name,
      message: cause.message && cause.message !== error.message ? truncateSingleLine(cause.message) : undefined,
    };
  }
...
function formatMarkdownMessage(summary: string, actions: readonly ErrorActionLink[] | undefined = undefined): string {
  const formattedSummary = `**${escapeBoldText(summary)}**`;
  const actionLinks = actions?.map(formatActionLink).join(' · ');
  return actionLinks ? [formattedSummary + '\\', '\\', `**${actionLinks}**`].join('\n') : formattedSummary;
}
...
function getErrorActions(error: DeepSeekRequestError, actionUrls: ErrorActionUrls): readonly ErrorActionLink[] {
...
function getHttpErrorActions(status: number, baseUrl: string, actionUrls: ErrorActionUrls): readonly ErrorActionLink[] {
  return [
...
function getUniversalHttpErrorActions(status: number, actionUrls: ErrorActionUrls): readonly ErrorActionLink[] {
...
function getProviderHttpErrorLink(status: number, baseUrl: string): HttpErrorLinkDefinition | undefined {
...
function truncateSingleLine(value: string): string {
  ...
  return singleLine.length > MAX_DIAGNOSTIC_FIELD_LENGTH ? `${singleLine.slice(0, MAX_DIAGNOSTIC_FIELD_LENGTH)}...` : singleLine;
}
