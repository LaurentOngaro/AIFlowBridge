	...
	try {
		...
		if (rawModels.length === 0) {
			void vscode.window.showWarningMessage(
				`No models returned by ${VENDOR_LABELS[vendor]} (${baseUrl}/models). Check your API key and try again.`,
			);
			return;
		}
		...
		if (candidates.length === 0) {
			void vscode.window.showInformationMessage(
				`All models from ${VENDOR_LABELS[vendor]} are already in your userModels.`,
			);
			return;
		}
		...
		const toolCalling = await vscode.window.showQuickPick(
			[{ label: 'Yes' }, { label: 'No' }],
			{ title: 'AIFlowBridge: Tool calling support?', placeHolder: 'Does this model support tool calling?' },
		);
		if (!toolCalling) return;
		const imageInput = await vscode.window.showQuickPick(
			[{ label: 'Yes' }, { label: 'No' }],
			{ title: 'AIFlowBridge: Vision support?', placeHolder: 'Does this model support image input?' },
		);
		if (!imageInput) return;
		const thinking = await vscode.window.showQuickPick(
			[{ label: 'Yes' }, { label: 'No' }],
			{ title: 'AIFlowBridge: Thinking support?', placeHolder: 'Does this model support thinking / reasoning?' },
		);
		if (!thinking) return;
		const capabilities = {
