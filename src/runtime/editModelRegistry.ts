...
import { BUNDLED_REGISTRY_RELATIVE_PATH, GLOBAL_STORAGE_REGISTRY_RELATIVE_PATH } from '../aiflowbridge/modelRegistry';
...
export async function editModelRegistryCommand(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (!(await fileExists(globalStorageUri))) {
    try {
      ...
      logger.info(`[AIFlowBridge] Initialized model registry override at ${globalStorageUri.toString()} from bundled defaults`);
    } catch (err) {
      logger.error('[AIFlowBridge] Failed to initialize model registry override', err);
      void vscode.window.showErrorMessage(`Failed to initialize model registry override: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
  }
