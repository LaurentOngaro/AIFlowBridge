...
export async function resetModelRegistryCommand(context: vscode.ExtensionContext): Promise<void> {
	...
	if (!(await fileExists(globalStorageUri))) {
		void vscode.window.showInformationMessage(
			'No model registry override to reset - the bundled defaults are already in effect.',
		);
		return;
	}
	...
	try {
		...
		void vscode.window.showErrorMessage(
			`Failed to delete model registry override: ${err instanceof Error ? err.message : String(err)}`,
		);
		return;
	}
