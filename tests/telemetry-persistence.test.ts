...
import { acquireTelemetryLock, defaultTelemetryPaths, releaseTelemetryLock, TelemetryPersister } from '../src/aiflowbridge/telemetry/persistence';
...
describe('TelemetryPersister.loadSync', () => {
  ...
  // regression: a 2.6.0 / 2.5.1 user upgrading from a version
...
describe('TelemetryPersister.appendDelta', () => {
  ...
  it('accumulates N distinct entries into a single on-disk snapshot', async () => {
    ...
    for (let i = 0; i < total; i++) {
      await persister.appendDelta(makeEntry({ id: `r${i}`, model: `m${i}` }), emptyTelemetrySnapshot());
    }
    const loaded = persister.loadSync();
  ...
  it('survives 50 parallel writers with no lost updates', async () => {
    const N = 50;
    const calls = Array.from({ length: N }, (_, i) => persister.appendDelta(makeEntry({ id: `r${i}`, model: `m${i}` }), emptyTelemetrySnapshot()));
  ...
  it('does not leave a partial file observed by a concurrent reader', async () => {
    ...
    const N = 30;
    await Promise.all(
      Array.from({ length: N }, (_, i) => persister.appendDelta(makeEntry({ id: `r${i}`, totalTokens: 1000 + i }), emptyTelemetrySnapshot()))
    );
...
describe('TelemetryStore + persister integration', () => {
  ...
  it("removeEntry() reverses the entry's deltas in the on-disk snapshot", async () => {
    await persister.appendDelta(
      makeEntry({ id: 'r1', totalTokens: 30, promptTokens: 10, completionTokens: 20, durationMs: 100, estimatedCost: 0.001 }),
      emptyTelemetrySnapshot()
    );
    await persister.appendDelta(
      makeEntry({ id: 'r2', totalTokens: 70, promptTokens: 25, completionTokens: 45, durationMs: 300, estimatedCost: 0.002 }),
      emptyTelemetrySnapshot()
    );
