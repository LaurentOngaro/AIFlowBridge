/**
 * Unit tests for the UX-related command regressions.
 * introduced after the  refactor. The runtime is exercised
 ...
 *
 * Covered regressions:
 *   `aiflowbridge.resetMetrics` asks for confirmation via
 *         `ctx.confirm` and aborts on anything other than "Reset".
 *   `aiflowbridge.copyGatewayUrl` writes the URL via
 *         `ctx.clipboardWrite` (or to stdout when the hook is
 *         absent, e.g. in standalone mode).
 *   `aiflowbridge.openSettings` delegates to
...
function makeMockHost(): MockHost {
  ...
  return {
    ...
    set confirmResult(value: string | undefined) {
      confirmResult = value;
    },
    clipboardWrites,
    openSettingsCalls,
...
describe('AIFlowBridgeRuntime - UX command regressions (.)', () => {
  let host: MockHost;
  ...
  it('resetMetrics prompts via ctx.confirm and aborts on Cancel', async () => {
  ...
  it('resetMetrics prompts via ctx.confirm and proceeds on Reset', async () => {
    const handler = host.commands.get('aiflowbridge.resetMetrics');
    expect(handler).toBeDefined();
  ...
  it('copyGatewayUrl writes the gateway URL to ctx.clipboardWrite', async () => {
  ...
  it('openSettings delegates to ctx.openSettings scoped to aiflowbridge', async () => {
