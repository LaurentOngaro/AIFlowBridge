...
describe('validateModelEntry (fail-soft)', () => {
  ...
  it('rejects unsupported pricing currency', () => {
    const log: ValidationLog = { skipped: [] };
    expect(
      validateModelEntry(validEntry({ pricing: { inputPerMillion: 0.27, outputPerMillion: 1.1, currency: 'EUR' as never } }), 0, log)
    ).toBeNull();
  ...
  it('rejects negative pricing numbers', () => {
    const log: ValidationLog = { skipped: [] };
    expect(validateModelEntry(validEntry({ pricing: { inputPerMillion: -1, outputPerMillion: 1.1, currency: 'USD' } }), 0, log)).toBeNull();
    expect(validateModelEntry(validEntry({ pricing: { inputPerMillion: 0.27, outputPerMillion: -1, currency: 'USD' } }), 0, log)).toBeNull();
  });
});
...
describe('validateModelEntry (partial mode - override tiers)', () => {
  ...
  it('still rejects invalid pricing when explicitly provided', () => {
    const log: ValidationLog = { skipped: [] };
    expect(validateModelEntry({ id: 'm1', pricing: { inputPerMillion: -1, outputPerMillion: 1, currency: 'USD' } }, 0, log, 'partial')).toBeNull();
...
describe('validateVendorEntry (fail-soft)', () => {
  ...
  it('rejects invalid externalUrls', () => {
    expect(validateVendorEntry({ baseUrl: 'https://x', apiKeySecret: 'k', externalUrls: 'oops' })).toBeNull();
    expect(validateVendorEntry({ baseUrl: 'https://x', apiKeySecret: 'k', externalUrls: null })).toBeNull();
  });
});
...
describe('Deep merge', () => {
  ...
  it('mergeTiers: later tier wins, fields not overridden fall through', () => {
    ...
    const t2: ReturnType<typeof validateRegistryContent> = {
      ...
      models: [{ ...baseModel(), detail: 'override' }],
      log: { skipped: [] },
    };
