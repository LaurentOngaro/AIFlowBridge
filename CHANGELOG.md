...
- **Google AI Studio BYOK (native surface, default route).** New pure modules under `src/aiflowbridge/antigravity/`: `gemini-native.ts` (OpenAI ↔ Gemini native translation), `googleai-studio-route.ts` (route switcher logic), `modelRegistryOverride.ts` (atomic override cleanup). The gateway translates requests to `:generateContent` / `:streamGenerateContent?alt=sse` on `generativelanguage.googleapis.com` and authenticates with the `x-goog-api-key` header (the OpenAI-compat `/openai/chat/completions` surface is feature-gated per GCP project and returns 429 with zero quota on projects that have not opted in). `aiflowbridge.providers.googleaistudio.apiKey` is read from `AIFLOWBRIDGE_GOOGLEAISTUDIO_API_KEY` env var, `<globalStorageDir>/secrets.json`, or VS Code `SecretStorage`. The standalone CLI adds `aiflowbridge-server auth googleaistudio setApiKey <AIzaSy...>` and `clearApiKey`; the extension adds the corresponding `Google AI Studio: Set API Key (BYOK pay-as-you-go)` and `Clear API Key` commands. Bundled models carry the real upstream public-API rates ($0.30 / $2.50 per 1M USD) and `GET /v1/models` advertises them to any OpenAI-compatible client.
- **Google AI Studio / Antigravity via Cloud Code Assist OAuth (advanced).** Ten new pure modules under `src/aiflowbridge/antigravity/` (`constants.ts`, `types.ts`, `pkce.ts`, `envelope.ts`, `sse-transform.ts`, `token-store.ts`, `auth.ts`, `project.ts`, `catalog.ts`, `index.ts`): Google Authorization Code + PKCE flow with a loopback callback server, token persistence in `<globalStorageDir>/secrets.json` (chmod 600) with auto-refresh, Cloud Code project resolution via `loadCodeAssist`, model discovery, OpenAI-to-Cloud-Code envelope translation, and Cloud-Code-SSE to OpenAI chunk conversion for streaming plus single-object accumulation for non-streaming. The gateway routes `kind: 'googleaistudio'` profiles through the token manager with the Antigravity `User-Agent` + `X-Goog-Api-Client` headers. Works only for accounts with a Cloud Code Assist whitelisted tenant.
- **One-click OAuth login that opens the browser.** `AIFlowBridge: Connect to Google AI Studio` (extension) and `aiflowbridge-server auth googleaistudio` (standalone, with `--status` and `--logout`) open the Google consent page in the default browser via a new `IGatewayContext.openExternal` hook (`vscode.env.openExternal` for the extension, `xdg-open` / `open` / `start` for the CLI). The clipboard copy and the printed URL remain as fallback for headless environments. `AIFlowBridge: Disconnect from Google AI Studio` revokes the locally stored tokens.
- **Token plans vs per-token billing in the dashboard.** New optional `billing` field on `aiflowbridge.providers[]` entries (`token` default, `plan` when usage is covered by a token plan, subscription, or OAuth plan) plus `aiflowbridge.providers.googleaistudio.baseUrl` / `maxTokens` settings. OAuth kinds are always plan-covered; MiniMax token-plan keys opt in explicitly. Every recorded request is stamped (`RequestTelemetry.billedTo`), the Recent table shows a `plan` badge with a `(plan)` cost suffix and tooltip, a billing notice under the headline cards splits the plan-covered share of the total, the Filters search accepts `plan` / `token`, and CSV / JSON exports carry a `billedTo` column. The Est. cost formula is unchanged: for plan rows it is the indicative pay-as-you-go equivalent, not a real charge.
- **`AIFlowBridge: Switch Google AI Studio route` command.** Toggles `aiflowbridge.providers.googleaistudio.baseUrl` between the two bundled defaults (`generativelanguage.googleapis.com/v1beta` for BYOK, `cloudcode-pa.googleapis.com` for Antigravity OAuth), clears the credentials of the inactive route (revokes OAuth tokens when leaving AGY, clears the API key when leaving BYOK), and strips any stale `vendors.googleaistudio` override from `<globalStorageDir>/models.json` (which would otherwise silently hijack the new baseUrl because the 3-tier model registry merge has globalStorage override priority over the bundled tier). The companion `Restart gateway here (debug)` command helps debug sessions that joined the launching VS Code's gateway and need to reload fresh code.

### Changed

- **`synthesizeProviderForModel` derives the gateway `kind` from the registry family.** `src/aiflowbridge/host-config.ts` - `googleaistudio` / `antigravity` families synthesize with their own kind (plus `billing: 'plan'`) instead of being flattened to `openai-compat`, so Kilo Code / Continue see routable Gemini entries in the catalog. The `hand-curated gateway profiles` guard and the `preserves the antigravity kind` regression test pin the behavior.
- **Streaming pipeline refactored to drain-then-transform.** `src/aiflowbridge/gateway/server.ts` - the upstream body is now drained to completion (`drainReadableStream`), the SSE transform is applied to the buffered bytes (`accumulateThroughTransform`), and the OpenAI-shaped output is replayed to the client via a single `Readable.fromWeb`. Pure `webStream.pipeThrough(transform)` lost bytes in production because the transform's `flush()` ran with an empty buffer under Node Web Streams backpressure, leaving clients with `[DONE]`-only frames. Note: this trades per-chunk time-to-first-token for byte-correctness; the time-to-first-token regression for large generations is tracked in the audit v2 backlog (BUG-17) for 2.18.0.
- **Docs refresh.** `README.md`, `docs/providers.md` (BYOK + AGY sections + plan vs per-token billing table), `docs/cost.md` (Google AI Studio family row + billing posture column), `docs/dashboard.md` (plan badge + notice + exports), `docs/standalone.md` (`auth googleaistudio [setApiKey|clearApiKey|--status|--logout|--probe|--list-models]`), `docs/gateway.md` (Kilo Code / Continue model list including the three Gemini ids).

### Fixed

- **Antigravity SSE bugs.** `src/aiflowbridge/antigravity/sse-transform.ts` - stable `tool_calls` ids derived from the completion id + call index (chunked function calls accumulate correctly client-side), `flush()` emits text + tool calls + finish reason + usage + error frames like `transform()` instead of text only, `accumulateAntigravityResponse` wraps the read loop in try/catch with `reader.releaseLock()`.
- **OAuth robustness.** `src/aiflowbridge/antigravity/auth.ts` - upstream error bodies are sanitized before being thrown (no token material in logs), the loopback flow times out after 5 minutes instead of hanging forever, missing `access_token` in refresh / exchange responses is rejected explicitly, the browser error page no longer echoes the raw error param, and single-read response parsing in `project.ts` avoids masking the original failure.
- **Native API key header.** `buildGeminiNativeUpstreamRequest` now sends `x-goog-api-key: <AIzaSy...>` instead of `Authorization: Bearer <AIzaSy...>`. The native Gemini endpoint reserves the Bearer form for OAuth 2.0 access tokens; sending an API key under Bearer returns 401 `ACCESS_TOKEN_TYPE_UNSUPPORTED`. The OAuth / AGY branch keeps Bearer (correct for access tokens).
- **Native tool calling.** `createGeminiNativeToOpenAiSseStream` now emits OpenAI-style `delta.tool_calls` chunks with stable per-call ids derived from `completionId + toolCallIndex`; `fromGeminiNativeResponse` maps `parts[].functionCall` into `choices[0].message.tool_calls`. Both paths previously dropped function calls entirely, breaking agent loops.
- **Native finish_reason mapping.** `mapGeminiFinishReason()` translates `STOP` → `stop`, `MAX_TOKENS` → `length`, `SAFETY` / `RECITATION` / `BLOCKLIST` / `PROHIBITED_CONTENT` / `SPII` → `content_filter`. The previous hardcoded `stop` masked truncation and safety blocks.
- **Native URL rewrite.** `resolveUpstreamUrl()` injects the `/openai/` prefix when the upstream host is `generativelanguage.googleapis.com` and the path does not already start with `/openai/`. This routes the OpenAI-compat surface on the public Gemini API to the documented endpoint; without the rewrite the gateway POSTed to a non-existent path.
- **Native streaming 4xx guard.** The streaming error branch now triggers on `statusCode >= 400` (not only 429 / 503). Gemini's native and Cloud Code Assist surfaces return non-2xx errors with `content-type: text/event-stream` and a JSON body; the previous guard left 400 / 401 / 403 / 404 errors unhandled and the gateway piped the JSON body as SSE frames, leaving clients with `[DONE]`-only streams.
- **Native schema cleanup.** `cleanJsonSchema()` now strips `exclusiveMinimum` and `exclusiveMaximum` from tool parameter schemas (the Gemini native surface rejects these OpenAPI-3.x keywords with `Unknown name ... Cannot find field`).
- **API key env map and command wiring.** `SECRET_KEY_TO_ENV_NAME` recognizes `AIFLOWBRIDGE_GOOGLEAISTUDIO_API_KEY`; `SECRET_SHORT_TO_FULL` recognizes the `googleaistudio.apiKey` short form. The VS Code `setApiKey` / `clearApiKey` commands are now actually registered (the previous build declared them in `package.json#contributes.commands` but registered no handler, returning `command not found` from the Command Palette).
- **OAuth init order.** `src/aiflowbridge/index.ts` instantiates `tokenManager` before `logApiKeySources()` so the startup OAuth-status log line is meaningful instead of always printing `not connected`.
- **Token store cleanup.** `AntigravityTokenStore.save()` no longer writes the same serialized JSON blob under both `antigravity` and `googleaistudio` keys in `secrets.json`. The `googleaistudio` slot is now reserved for the BYOK API key string; OAuth tokens live under `antigravity`. Stale JSON blobs under the old alias are cleaned up on save.
- **Registry validation for the new vendor.** `KNOWN_FAMILIES` (`src/aiflowbridge/modelRegistry.schema.ts`) and the `family` enum (`resources/models.schema.json`) accept `antigravity` / `googleaistudio`, so the bundled Gemini entries load instead of being skipped; `VENDOR_CHOICES` (`src/runtime/addCustomModel.ts`) lists Google AI Studio and the `kind` enum in `package.json` accepts the new values.
...
- **`forwardChatCompletion` decomposed into three pure helpers.** `src/aiflowbridge/gateway/server.ts` - the 715-line chat-completion handler is now a thin orchestrator (~570 lines) that delegates to three private methods: `readAndValidateBody()` (body read + JSON parse + 400 on failure), `resolveChatProvider()` (prompt summary capture + model name + `selectProviderWithLanguage` + 503/404 on failure), and `buildUpstreamRequest()` (upstream URL + API-key resolve + bearer-key shape check + headers + OpenRouter attribution + payload translation + workspace-context injection + provider model override + 502 on bearer rejection). The orchestrator now owns only the request-lifecycle concerns (in-flight counter, abort controller, per-provider slot, watchdog timers, streaming / non-streaming / backoff / catch / finally). No behavior change for callers; the per-handler split makes each step individually testable. All 1038 unit tests pass (56 files), `npm run compile` + `npm run compile:standalone` + `npm run typecheck:tests` return 0 errors.
- **`GET /v1/models` now exposes the bundled pricing snapshot freshness as response headers.** `X-AIFlowBridge-Pricing-GeneratedAt` + `X-AIFlowBridge-Pricing-Version` carry the same `generatedAt` / `aiflowbridgeVersion` pair that already drives the dashboard `Est. cost` tooltip, so external OpenAI-compatible clients (Kilo Code, Continue, Open WebUI) can cache the catalog intelligently. Headers are omitted when the pricing registry is empty / pre-2.15.0 so backward compatibility is preserved.
- **`AIFLOWBRIDGE_CONFIG_WATCH_INTERVAL_MS` env var exposes the standalone config-file polling interval.** `src/standalone/context.ts` - operators on slow disks (NFS, WSL2, network mounts) can now raise the polling interval above the 5 s default without a code change. Out-of-range or non-numeric values fall back to the 5 s default so a typo never silently disables the watcher. Minimum effective interval 250 ms (below that the polling loop outpaces mtime granularity and burns CPU for no benefit).
- **`BLOCKED_HOSTS` SSRF guard documents why `0.0.0.0` and `::` are intentionally allowed.** `src/aiflowbridge/providers.ts` - extended the block-list JSDoc to explain that bind-all addresses are NOT metadata endpoints and that blocking them would break the legitimate Docker `--net=host` workflow (Ollama / vLLM / llama.cpp servers bound to every interface). A future contributor tempted to add them "to be safe" now has a documented design choice to revert.
- **Audit recommendations tracked with checkboxes in the audit doc.** `docs/audits/2026-08-06-audit-v2.15.5.md` gains an "Implementation status" table at the top of section 1 so the next audit pass can see at a glance which recommendations have landed and which are deferred.

### Fixed

- **`/v1/chat/completions` removeEntry left on-disk `byClient` / `bySource` stale until the next retention prune.** `src/aiflowbridge/telemetry/persistence.ts` - `revertEntryFromSnapshot()` now reverses the per-originating-client and per-origin maps symmetrically with the `byProvider` / `byModel` reversal it already did. Before the fix, the cross-window mirror decremented the provider and model buckets on disk but left the client and source buckets stale until a retention prune rebuilt them via `rebuildSnapshot`. A peer window that called `refreshFromDisk()` after a delete would re-derive them correctly in memory via `rebuildSnapshot`, so the on-disk counters were the only stale view.
- **Unicode ellipsis (U+2026) in user-facing prose and source comments replaced with ASCII `...`.** `src/aiflowbridge/telemetry/persistence.ts:636` (JSDoc for `revertEntryFromSnapshot`) and `docs/dashboard.md:68` (Filters panel description) - both flagged by `style.md` as ASCII-only typography violations and caught by `markdownlint` MD026 in the project's prose guard.

### Added

- **`deepseek-flash` / `deepseek-pro` whitelist tests pin the historical DeepSeek alias convention.** `tests/host-config.test.ts` - two new regression tests in the `hand-curated gateway profiles use real upstream model ids as catalog ids` describe block: (1) the alias is preserved through `synthesizeProvidersFromBuiltInModels()` (the synthesis would otherwise drop it because the registry doesn't carry the friendly alias), (2) `selectProvider()` still routes both the alias (`deepseek-flash`) and the upstream id (`deepseek-v4-flash`) to the same provider so a Kilo Code / Continue config that pinned the friendly alias keeps working. The historical DeepSeek alias convention (`id: 'deepseek-flash'`, `model: 'deepseek-v4-flash'`) is documented as the allowed exception to the `id === upstream model id` invariant in `docs/agent-instructions/tasks.md` (Path A, step 9); without the whitelist a future contributor could "fix" the alias to match `model`, rename the catalog id, and break every config that pinned the friendly alias in the picker.
